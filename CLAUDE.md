...
## Estado
2026-08-01 — **Escrituras a Supabase ya no fallan en silencio + migración 027 lista (commit `969dcba`, EN PROD).** Los 3 commits de estas sesiones (`d7cd0d0`, `f591aab`, `969dcba`) están pusheados. (1) **Regla nueva: toda escritura optimista avisa al fallar** — 7 de las 9 rutas (clientes/tareas/reuniones × crear/editar/borrar) solo hacían `console.warn`, así que un fallo dejaba al usuario creyendo que guardó; helper `onWriteError()` en `useClientStore` aplicado a las 9. (2) **Migración `027_tasks_updated_at.sql` escrita pero ⚠️ SIN CORRER** — agrega `updated_at` + trigger `tasks_touch` a `tasks` (reusa `touch_updated_at()` de clients/projections). No urge: nada lee `task.updatedAt` hoy; sirve para desactivar la mina, porque `taskToRow()` sigue mapeando el campo. (3) Playwright MCP probado también en **caminos de error** (interceptar `fetch` → 400 forzado) sin tocar datos reales. Pendientes: **correr la 027**; probar **logout→login** (requiere credenciales); **code-splitting** del bundle de 4 MB — plan en 2 pasos (`manualChunks` primero, `React.lazy` por ruta después), dejado para sesión propia por ser lo más riesgoso.
